...
const ResizeableContainer = ({ children, onDelete, position, sizePass, onSizePass, onPosition, div2Ref, index, isShowBoundry }) => {
    ...
    const moveHandler = (mouseDownEvent) => {
        ...
        function onMouseMove(mouseMoveEvent) {
            ...
            const tempTop = mouseMoveEvent.y - frameBoundry.y - (sttickerBoundry.height / 2)
            const tempLeft = mouseMoveEvent.x - frameBoundry.x - (sttickerBoundry.width / 2)
            setMousePosition({
                left: tempLeft,
                top: tempTop
            });
    ...
    const touchMoveHandler = (ee) => {
        ...
        function onMouseMove(e) {
            ...
            const frameBoundry = div2Ref.current[index].getBoundingClientRect()
            const tempTop = mouseMoveEvent.clientY - frameBoundry.y - (sttickerBoundry.height / 2)
            const tempLeft = mouseMoveEvent.clientX - frameBoundry.x - (sttickerBoundry.width / 2)
            setMousePosition({
                left: tempLeft,
                top: tempTop
            });
    ...
    return (
        <div id={styles.resizeContainer} style={{
            ...
            <img id={styles.draghandle} src={ResizeIcon} width={12} draggable="false" onMouseDown={handler} onTouchStart={touchHandler}
                style={isShowBoundryStyle}
            />
            <img onClick={() => onDelete()} id={styles.deleteIcon} src={RemoveIcon} width={16} draggable="false" style={isShowBoundryStyle} />
        </div>
    );
}
