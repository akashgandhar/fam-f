...
const GiftCardContainer = () => {
    ...
    useEffect(() => {
        ...
        const fetchFrameSizes = async () => {
            try {
                const response = await axios.get('https://backend.familyvibes.in/getGiftCardFrames');
    ...
    async function buyFramesToPhonePay(initPayment) {
        ...
        try {
            const result = await authAxios.post('https://backend.familyvibes.in/promo/orderId', initPayment, { headers: header });
            const { data } = result;
            if (data.success) {
                const { message } = data;
