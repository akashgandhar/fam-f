...
const NavContainer = () => {
  ...
  useEffect(() => {
      ...
      [
        { transform: "translateX(98%)" },
        {
          transform: `translateX(-${
            window.innerWidth + refElement.scrollWidth
          }px)`,
        },
      ],
      {
    ...
    return () => {
      animation?.cancel();
      refElement?.parentElement?.removeEventListener(
        "mouseenter",
        handleMouseEnter
      );
      refElement?.parentElement?.removeEventListener(
  ...
  return (
    <>
      <NavHashLink
        to="#bodyTop"
        className={`btn btn-orange btn-square ${isSticky ? "show" : "hide"}`}
      >
        <svg
      ...
      <span className="btn-frame">
        <NavLink id="sfr" to="/frames" className={"btn btn-14"}>
          Buy Now
        </NavLink>
      </span>
      <div className="headerContainer" id="bodyTop">
        <header
          ref={navbarRef}
          className={isSticky ? "bg-fixed-grad sticky" : "bg-fixed-grad"}
          id="navbar"
        >
                ...
                {homeData?.offer?.map((discount, index, array) => {
                  return (
                    <span key={index + 15}>
                      ...
                      <strong>₹{discount.minimumAmount}</strong> By using{" "}
          ...
          <div className="container nav-container d-flex justify-content-between align-items-center py-3">
            ...
            <nav>
              <div className="nav-mobile">
                <a id="navbar-toggle" onClick={toggleNavbar}>
                  <span></span>
                </a>
              </div>
              <ul className="nav-list" id="navList">
                ...
                <li>
                  <NavHashLink to="/#gift_cards" className="t-dark" id="gf">
                    Gift Cards
                  </NavHashLink>
                </li>
                <li>
                  <NavHashLink to="/blogs" className="t-dark" id="gf">
                    Blogs
                  </NavHashLink>
                </li>
                ...
                <li>
                  {profile ? (
                    <button
                      className="border-0 bg-transparent t-theme"
                      onClick={() => {
                        if (profile) {
                          googleLogout();
                    ...
                    <button
                      className="btn btn-orange rounded-pill swingAnimate"
                      id="lg"
