...
const Frames = () => {
  ...
  const centerItem =
    width < 786
      ? images?.length < 2 && { justifyContent: "center" }
  ...
  const [numberOfFrames, setNumberOfFrames, framesPreset, setFramesPreset] =
    useFrameContext();
  ...
  const createButton = () => (
    <div className="text-center">
      {profile ? (
        ...
        <>
          <div className="my-4">
            ...
            <button
              className="ml-3 btn btn-orange rounded-pill swingAnimate"
              onClick={() => dispatch(showLoginAction(true))}
            >
              Login Now
            </button>
          </div>
      ...
      <div
        ...
        className="frame-scale d-inline-block mt-3"
      >
        <svg
      ...
      // If the count doesn't match, handle it appropriately (add/remove frames)
        ...
        if (parsedImages.length < numberOfFrames) {
          ...
          const newFrames = Array.from(
            { length: numberOfFrames - parsedImages.length },
            (_, index) => ({
              width: frameWidth,
              height: frameHeight,
      ...
      <section className="h-100">
        <div className="frame-main h-100">
          <div id="mainFrameScrollContainer" className="h-100">
            {initialFramesPreset != 0 &&
              initialFramesPreset > numberOfFrames && (
                <div className="alert alert-danger text-center">
                  You have to add {initialFramesPreset - numberOfFrames} more
                  frames to complete the order.
            ...
            {images?.length > 0 && (
              <BottomSelector
                availableColors={
                  sizes?.find((item) => item?.size === size1)?.colors
                }
                setImages={setImages}
                ...
                updateRange={(rangeValue) => {
                  if (imgRef.current && imgRef.current.length > 0) {
                    if (selectedFrame > -1) {
                      const splitt =
                        globalImages[selectedFrame].scale.split(" ");
                      let tempTrans = splitt[0];
                      if (rangeValue == "1" || rangeValue == 1) {
                        tempTrans = "translate(0px,0px)";
                      }
                      const value = `${tempTrans} rotate(0deg) scale(${
                        1 + rangeValue / 10
                      })`;
                      imgRef.current[selectedFrame].style.setProperty(
                        "transform",
                        value
                      );
                      ...
                      for (let item in imgRef.current) {
                        ...
                        imgRef.current[item].style.setProperty(
                          "transform",
                          value
                        );
                ...
                updateMat={(mat) => {
                  if (imgRef.current && imgRef.current?.length > 0) {
                    if (selectedFrame > -1) {
                      const splitt =
                        globalImages[selectedFrame].div3Class.split(" ");
                      div3Ref.current[
                        selectedFrame
                      ].className = `${mat} ${splitt[1]}`;
            ...
            {images?.length == 0 && initialFramesPreset < 1 ? (
              createButton()
            ) : (
              //  {images?.length === 0 ? createButton() :
              <>
                <div
                  className="EmptyFramesRow"
                ...
                {images.map((item, index) => (
                  <FrameContainer
                    ...
                    onDelete={async (index) => {
                      const tempData = globalImages.filter(
                        (item, subIndex) => subIndex !== index
                      );

                      // Handle file unlinking
                      await handleUnlinkFile(globalImages?.[index]?.url);
                      globalImages?.[index]?.frameUrl &&
                        (await handleUnlinkFile(
                          globalImages?.[index]?.frameUrl
                        ));
                      globalImages?.[index]?.original_image &&
                        (await handleUnlinkFile(
                          globalImages?.[index]?.original_image
                        ));
                      ...
                      const tempData = globalImages.map((item, subIndex) => {
                        ...
                        if (subIndex === index) {
                          const tempStickers = tempData.sticker.filter(
                            (stickerItem, index) => index !== sticketIndex
                          );
...
export const BottomSelector = ({
  ...
  useEffect(() => {
    if (frame != -1) {
      frameClassGloable =
        frame === 0 ? "frame-one" : frame === 1 ? "frame-two" : "frame-three";
      subFrameGloable =
        frame == 1
          ? "sub-frame-inner sub-frame"
          : frame === 2
          ? "sub-frame-inner sub-frame-wood"
          : "sub-frame-inner";
  ...
  if (type === "frame") {
        ...
        {colors.map((color) => (
          <div
            ...
            className={`toolContent ${
              selectedColor === color.hex ? "activeEffect" : ""
            }`}
            onClick={() => {
              setColor1(color);
    ...
    return (
      <div className="ToolBox Effect">
        ...
        <div
          className={`toolContent ${
            effect === "filter-willow" ? "activeEffect" : ""
          }`}
        ...
        <div
          className={`toolContent ${
            effect === "filter-inkwell" ? "activeEffect" : ""
          }`}
        ...
        <div
          className={`toolContent ${
            effect === "filter-aden" ? "activeEffect" : ""
          }`}
        ...
        <div
          className={`toolContent ${
            effect === "filter-dogpatch" ? "activeEffect" : ""
          }`}
        ...
        <div
          className={`toolContent ${
            effect === "filter-earlybird" ? "activeEffect" : ""
          }`}
